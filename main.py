...
<head>
  ...
  <title>MinePulse AI - Industrial Compliance</title>
      ...
      document.getElementById('ol').innerHTML = obs.map(o => `
        <div class="p-2.5 bg-slate-950 border border-slate-800 rounded flex justify-between items-center">
          <div>
            ...
            <div class="text-[10px] text-slate-500 font-mono">HASH: ${o.sha256_hash ? o.sha256_hash.slice(0, 16) : 'LEGACY'}...</div>
          </div>
          <div class="text-right">
            <span class="text-slate-400 text-[10px] block">${o.timestamp}</span>
            <span class="text-[10px] text-emerald-400 font-mono">SEAL INTACT</span>
          </div>
        </div>
...
# --- DGMS FORM-VI STATUTORY EXPORT ROUTE ---
