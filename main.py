...
FORM_VI_HTML_BASE = """<!DOCTYPE html>
<html>
<head>
...
<body>
    <div class="header">
        <h2>DIRECTORATE GENERAL OF MINES SAFETY (DGMS)</h2>
        <h3>STATUTORY INSPECTION & BREACH REGISTER (FORM-VI)</h3>
        <p>Under Coal Mines Regulations (CMR 2017) & Mines Act 1952</p>
    </div>
...
def add_inspection(b: BatchItem):
    ...
    try:
        ...
        now = datetime.utcnow()
        raw = str(b.mine_id) + ":" + str(b.officer_name) + ":" + str(b.dgms_cert_no) + ":" + str(b.notes) + ":" + str(b.latitude) + ":" + str(b.longitude) + ":" + str(now)
...
def export_form_vi():
    ...
    try:
        ...
        if not audits:
            table_body = "<tr><td colspan='7' style='text-align:center; padding: 15px;'>No statutory breaches logged.</td></tr>"
        else:
            rows = []
            for a in audits:
                t_str = a.timestamp.strftime("%d/%m/%Y %H:%M") if a.timestamp else "N/A"
                m_name = str(a.mine_name or "Colliery")
                o_info = str(a.officer_name or "Officer") + " (" + str(a.officer_role or "Inspector") + ")"
                cat = str(a.category or "Statutory")
                notes = str(a.notes or "")
                sev = str(a.severity or "Normal")
                h_val = str(a.sha256_hash or "N/A")[:16] + "... [VERIFIED]"
                ...
                tr += "<td>" + notes + "</td>"
                tr += "<td>" + sev + "</td>"
                tr += "<td style='font-family: monospace; color: green;'>" + h_val + "</td>"
                tr += "</tr>"
                rows.append(tr)
