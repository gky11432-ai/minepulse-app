...
def init_db():
    ...
    try:
        ...
        if db.query(InspectionAudit).count() == 0:
            sample_time = datetime.utcnow()
            raw_h = f"SECL-GV-04:Er. Gaurav Yadav:DGMS/CMR/2017/OM-8492:Bench slope checked and stable:{sample_time}"
            s_hash = hashlib.sha256(raw_h.encode("utf-8")).hexdigest()
            db.add(InspectionAudit(
                id="CLI-INIT-01",
                mine_id="SECL-GV-04",
                mine_name="Gevra Opencast Sector B",
                officer_name="Er. Gaurav Yadav",
                officer_role="Safety Officer (Overman)",
                dgms_cert_no="DGMS/CMR/2017/OM-8492",
                category="CMR 106: Bench Stability & Slope",
                notes="Initial statutory baseline audit conducted. All benches within 45-degree angle.",
                severity="Normal",
                latitude=22.3541,
                longitude=82.6821,
