...
RAW_DB_URL = os.getenv("DATABASE_URL", "sqlite:///./minepulse.db")
...
class BatchItem(BaseModel):
    ...
    notes: str
    severity: str
    latitude: float = 22.3541
    longitude: float = 82.6821
...
@app.post("/api/telemetry-alert")
def auto_telemetry_alert():
    db = SessionLocal()
    try:
        mine = db.query(Colliery).filter(Colliery.id == "SECL-GV-04").first()
        now = datetime.utcnow()
        note = "AUTOMATED IOT DAEMON: Methane CH4 spike detected at 1.84% (Threshold: 1.25%). Immediate CMR 153 Stop Order."
        raw = f"SECL-GV-04:IoT Daemon:{note}:{now}"
        sha = hashlib.sha256(raw.encode("utf-8")).hexdigest()
        
        entry = InspectionAudit(
            id=f"TELEMETRY-{int(now.timestamp())}",
            mine_id="SECL-GV-04",
            mine_name=mine.name if mine else "Gevra Opencast Sector B",
            officer_name="Automated IoT Daemon",
            officer_role="Subsurface Optical Sensor",
            dgms_cert_no="DGMS/SYS/TELEMETRY-01",
            category="CMR 153: Ventilation & Methane Log",
            notes=note,
            severity="Critical",
            latitude=22.3541,
            longitude=82.6821,
        ...
        db.add(entry)
        if mine:
            mine.compliance_score = max(0.0, round((mine.compliance_score or 100.0) - 15.0, 1))
            mine.risk_level = "Critical"
        db.commit()
        trigger_emergency_dispatch("Gevra Opencast Sector B", note)
        return {"status": "TELEMETRY_ALERT_LOGGED", "sha256": sha}
    finally:
        db.close()
...
def export_form_vi():
    db = SessionLocal()
    try:
        audits = db.query(InspectionAudit).order_by(InspectionAudit.timestamp.desc()).limit(50).all()
        if not audits:
