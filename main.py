...
<body class="bg-slate-950 text-slate-100 min-h-screen font-sans flex flex-col">
  ...
  <main class="max-w-7xl mx-auto p-4 flex-1 w-full space-y-4">
    <div id="vd" class="space-y-4">
      <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div class="p-3 bg-slate-900 border border-slate-800 rounded-xl">
          <p class="text-[10px] text-slate-400">Dispatch Variance</p>
    ...
    <div id="vf" class="hidden max-w-lg mx-auto bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
      <h2 class="font-bold text-base">Field Inspection Form (Offline Enabled)</h2>
      <form onsubmit="save(event)" class="space-y-3 text-xs">
        ...
        <div class="grid grid-cols-2 gap-2">
          ...
          <div><label class="text-slate-400 block mb-1">GPS Lock</label><button type="button" onclick="lockGPS()" id="bgps" class="w-full bg-slate-800 p-2 rounded-lg text-emerald-400">Get Real GPS</button></div>
        </div>
  ...
  <script>
    ...
    async function load() {
      ...
      document.getElementById('ol').innerHTML = obs.map(o => `<div class="p-2 bg-slate-950 border border-slate-800 rounded flex justify-between"><div><strong>${o.mine_name}</strong>: ${o.notes}</div><div class="text-slate-400 text-[10px]">${o.timestamp}</div></div>`).join('') || '<p class="text-slate-500">No audits yet.</p>';
    }
    function lockGPS() {
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(p => {
          lat = p.coords.latitude; lng = p.coords.longitude;
          document.getElementById('bgps').innerText = lat.toFixed(3) + ', ' + lng.toFixed(3);
        });
      }
    }
    async function save(e) {
      ...
      alert('Committed to SHA-256 Audit Trail!');
    ...
    return [
        {
            ...
            "notes": a.notes, "severity": a.severity, "timestamp": a.timestamp.strftime("%Y-%m-%d %H:%M")
        }
        for a in res
    ]
