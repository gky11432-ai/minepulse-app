...
def trigger_emergency_dispatch(mine_name: str, issue: str):
    """Statutory Emergency Webhook: Broadcasts critical hazard to Telegram Safety Bot"""
    bot_token = "8824795075:AAEnJdgVkzhCfNsl7TWDi5HzYlXBa7txmS0"
    chat_id = "805617246"
    
    text = (
        f"🚨 *MINEPULSE DGMS CRITICAL BREACH*\n\n"
        f"📍 *Colliery:* {mine_name}\n"
        f"⚠️ *Statutory Hazard:* {issue}\n"
        f"⏱️ *Timestamp:* {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')} UTC\n\n"
        f"⚡ *Mandated Action:* Coal Mines Regulations 2017 Immediate Pit Isolation & Emergency Sirens Triggered."
    )
    ...
    try:
        ...
        print(f"[TELEGRAM DISPATCH] Alert delivered to Safety Officer phone.")
...
def add_inspection(b: BatchItem):
    ...
    # Live Telegram Push Notification
...
def export_form_vi():
    ...
    rows = "".join([
        f"""<tr style="border-bottom: 1px solid #ccc;">
            ...
            <td style="padding: 8px;">
                <div style="font-weight: bold;">{a.officer_name}</div>
                <div style="font-size: 11px; color: #555;">{a.officer_role}</div>
                <div style="font-family: monospace; font-size: 10px; color: #1e3a8a;">Lic: {a.dgms_cert_no}</div>
            </td>
            ...
            <td style="padding: 8px; font-family: monospace; font-size: 11px;">{a.sha256_hash[:16]}...<br><span style="color: green; font-weight: bold;">[VERIFIED]</span></td>
        </tr>"""
        for a in audits
    ])
    ...
    <body>
        ...
        <table>
            ...
            <tbody>
                {rows or "<tr><td colspan='7' style='text-align:center; padding: 20px;'>No statutory breaches logged in this cycle.</td></tr>"}
            </tbody>
        </table>
        <div class="footer">
            <div>
                <p>Generated by: <strong>MinePulse Statutory AI Engine</strong></p>
                <p>Certified Cryptographical
