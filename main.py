...
def trigger_emergency_dispatch(mine_name: str, issue: str):
    ...
    text = (
        ...
        + "Colliery: " + str(mine_name) + "\n"
        + "Hazard: " + str(issue) + "\n"
        + "Timestamp: " + str(now_time) + " UTC\n\n"
...
def export_form_vi():
    ...
    row_html = ""
    if not audits:
        row_html = "<tr><td colspan='7' style='text-align:center; padding: 20px;'>No statutory breaches logged in this cycle.</td></tr>"
    else:
        for a in audits:
            color = "red" if a.severity == "Critical" else "black"
            time_str = a.timestamp.strftime('%d/%m/%Y %H:%M') if a.timestamp else "N/A"
            row_html += "<tr style='border-bottom: 1px solid #ccc;'>"
            row_html += "<td style='padding: 8px; font-family: monospace;'>" + str(time_str) + "</td>"
            row_html += "<td style='padding: 8px; font-weight: bold;'>" + str(a.mine_name) + "</td>"
            row_html += "<td style='padding: 8px;'><div style='font-weight: bold;'>" + str(a.officer_name) + "</div><div style='font-size: 11px; color: #555;'>" + str(a.officer_role) + "</div><div style='font-family: monospace; font-size: 10px; color: #1e3a8a;'>Lic: " + str(a.dgms_cert_no) + "</div></td>"
            row_html += "<td style='padding: 8px;'>" + str(a.category) + "</td>"
            row_html += "<td style='padding: 8px;'>" + str(a.notes) + "</td>"
            row_html += "<td style='padding: 8px; color: " + color + "; font-weight: bold;'>" + str(a.severity) + "</td>"
            row_html += "<td style='padding: 8px; font-family: monospace; font-size: 11px;'>" + str(a.sha256_hash[:16]) + "...<br><span style='color: green; font-weight: bold;'>[VERIFIED]</span></td>"
            row_html += "</tr>"
