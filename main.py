...
def export_form_vi():
    ...
    try:
        ...
        rows_str = ""
        if not audits:
            rows_str = "<tr><td colspan='7' style='text-align:center; padding: 20px;'>No statutory breaches logged in this cycle.</td></tr>"
        else:
            for a in audits:
                ...
                rows_str += f"""<tr style='border-bottom: 1px solid #ccc;'>
                    <td style='padding: 8px; font-family: monospace;'>{t_str}</td>
                    <td style='padding: 8px; font-weight: bold;'>{m_name}</td>
                    <td style='padding: 8px;'>
                        <div style='font-weight: bold;'>{o_name}</div>
                        <div style='font-size: 11px; color: #555;'>{o_role}</div>
                        <div style='font-family: monospace; font-size: 10px; color: #1e3a8a;'>Lic: {o_cert}</div>
                    </td>
                    <td style='padding: 8px;'>{cat}</td>
                    <td style='padding: 8px;'>{notes}</td>
                    <td style='padding: 8px; color: {color}; font-weight: bold;'>{sev}</td>
                    <td style='padding: 8px; font-family: monospace; font-size: 11px;'>{h_val}...<br><span style='color: green; font-weight: bold;'>[VERIFIED]</span></td>
                </tr>"""
...
<body>
    <div class="header">
        ...
        <p style="margin: 0; font-size: 12px;">Under Coal Mines Regulations (CMR 2017) & Min
