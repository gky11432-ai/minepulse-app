    ...
    async function load() {
      try {
        ...
        document.getElementById('ol').innerHTML = (obs || []).map(o => {
          const vClass = o.is_valid ? 'text-emerald-400' : 'text-rose-500 font-bold';
          const vText = o.is_valid ? 'SEAL VERIFIED' : 'TAMPER DETECTED';
          return '<div class="p-2.5 bg-slate-900 border border-slate-800 rounded flex justify-between items-center">' +
            ...
            '<div class="text-right">' +
              ...
              '<span class="text-[10px] ' + vClass + ' font-mono">' + vText + '</span>' +
            '</div>' +
          '</div>';
...
def export_form_vi():
    ...
    try:
        ...
        if not audits:
            rows_html = "<tr><td colspan='7' style='text-align:center; padding: 20px;'>No statutory breaches logged in this cycle.</td></tr>"
        else:
            row_items = []
            for a in audits:
                colo
