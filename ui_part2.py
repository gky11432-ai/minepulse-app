# ui_part2.py - Autonomous Native Logic & Authentic DGMS Mine Air Siren
...
PART2 = """
  <script>
    ...
    var sirenOsc2 = null;
    var sirenGain = null;
    var sirenDistortion = null;
    var sirenMotorLoop = null;
    ...
    // 3. Official DGMS Mechanical Pit Siren (Air-Rotor Heavy Growl Model)
    function initAudio() {
      if (!audioCtx) {
    ...
    function triggerHardwareVibration() {
      if ('vibrate' in navigator) {
        navigator.vibrate([1200, 300, 1200, 300, 1800]);
        if (!vibInterval) {
    ...
    function playAudioSiren() {
      try {
        ...
        // Dual rotors for acoustic beating / phasing (mechanical air porting)
        sirenOsc1 = audioCtx.createOscillator();
        sirenOsc2 = audioCtx.createOscillator();
        sirenGain = audioCtx.createGain();
        sirenDistortion = audioCtx.createWaveShaper();

        // Mechanical distortion for harsh motor growl
        sirenDistortion.curve = makeDistortionCurve(12);
        sirenDistortion.oversample = '4x';

        sirenOsc1.type = 'sawtooth';
        sirenOsc2.type = 'triangle';

        // Initial motor spool-up frequency
        sirenOsc1.frequency.setValueAtTime(280, now);
        sirenOsc2.frequency.setValueAtTime(286, now); // Slight detune for harsh mechanical beating

        sirenGain.gain.setValueAtTime(0.45, now);

        sirenOsc1.connect(sirenDistortion);
        sirenOsc2.connect(sirenDistortion);
        sirenDistortion.connect(sirenGain);
        sirenGain.connect(audioCtx.destination);

        sirenOsc1.start(now);
        sirenOsc2.start(now);

        // DGMS Mine Motor Spool Cycle: 2.2s Wind Up, 1.8s Wind Down
        var spoolUp = true;
        var f = 280;

        sirenMotorLoop = setInterval(function() {
          if (!sirenOsc1 || !audioCtx) return;
          var t = audioCtx.currentTime;
          if (spoolUp) {
            f += 20;
            if (f >= 640) { spoolUp = false; }
          } else {
            f -= 16;
            if (f <= 280) { spoolUp = true; }
          }
          sirenOsc1.frequency.setTargetAtTime(f, t, 0.08);
          sirenOsc2.frequency.setTargetAtTime(f + 6, t, 0.08);
        }, 70);

        var b = document.getElementById('siren-banner');
        if (b) b.classList.remove('hidden');
      } catch(e) {
        console.error('DGMS siren engine error:', e);
      }
    }

    function silenceSiren() {
      if (sirenOsc1) {
        try { sirenOsc1.stop(); sirenOsc2.stop(); } catch(e){}
        sirenOsc1 = null;
        sirenOsc2 = null;
      }
      if (sirenMotorLoop) {
        clearInterval(sirenMotorLoop);
        sirenMotorLoop = null;
      }
      if (vibInterval) {
    ...
    function triggerBroadcastSOS() {
      ...
      alert('🚨 DGMS CMR 153 STATUTORY PIT EVACUATION ALARM ENGAGED!\\n\\nContinuous heavy mechanical air siren active.');
    }
