# ui_part2.py - Autonomous Native Logic & Real Danger Evacuation Siren
...
PART2 = """
  <script>
    ...
    // 3. Realistic Industrial Danger Siren Engine (Air-Raid Pitch Wail)
    function initAudio() {
      if (!audioCtx) {
    ...
    function playAudioSiren() {
      try {
        ...
        // Primary Siren Tone (Industrial Sawtooth)
        sirenOsc = audioCtx.createOscillator();
        var gainNode = audioCtx.createGain();
        sirenOsc.type = 'sawtooth';

        // Evacuation Wail: Ramp between 440 Hz (Danger Low) and 980 Hz (Piercing High)
        ...
        sirenInterval = setInterval(function() {
          ...
          sirenOsc.frequency.setTargetAtTime(currentFreq, t, 0.03);
        }, 50);

        // High gain for intense volume
        gainNode.gain.setValueAtTime(0.40, now);
    ...
    window.onload = async function() {
      // Audio engine ko pehle touch/click par instant unlock karega
