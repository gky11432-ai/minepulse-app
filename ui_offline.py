# ui_offline.py - Standalone Offline Storage, Audio Siren Synth & Network Sync Engine
  ...
  <script>
    ...
    async function checkTrueNetwork() {
      ...
      try {
        ...
        const resp = await fetch('/api/collieries', { method: 'HEAD', cache: 'no-store', signal: controller.signal });
        clearTimeout(timeoutId);
        if (resp.ok) setOnlineUI(); else setOfflineUI();
      } catch (err) {
        setOfflineUI();
      }
    }
