# ui_part1.py - Industrial UI with Always-Visible Print Form-VI Button
...
<body id="main-body" class="bg-slate-950 text-slate-100 min-h-screen font-sans flex flex-col transition-colors duration-300">
  ...
  <!-- Header with Visible Print Button & SOS Beacon -->
  <header class="border-b border-slate-800 bg-slate-900 sticky top-0 z-40 p-3 sm:p-4">
    <div class="max-w-7xl mx-auto flex justify-between items-center w-full">
      ...
      <div class="flex items-center space-x-1.5 sm:space-x-3">
        <!-- Always-Visible Print / Export Form-VI Button -->
        <a href="/statutory/form-vi" target="_blank" class="text-[11px] sm:text-xs px-2.5 sm:px-3 py-1.5 rounded-lg bg-amber-500/10 border border-amber-500/30 text-amber-400 font-bold flex items-center space-x-1 hover:bg-amber-500/20 transition">
          <span>📄</span>
          <span>Print Form-VI</span>
        </a>

        <!-- Minimal Compact Corner SOS Beacon -->
        <button onclick="triggerBroadcastSOS()" title="Trigger Mesh Emergency Siren" class="pulse-sos bg-rose-600 hover:bg-rose-500 text-white font-black text-xs px-2.5 sm:px-3 py-1.5 rounded-full border border-rose-300 shadow-md flex items-center space-x-1 cursor-pointer">
      ...
      <div class="bg-slate-900 border border-slate-800 rounded-xl p-4">
        <div class="flex justify-between items-center mb-3">
          <div>
            ...
            <p class="text-[11px] text-slate-400">Real-time DGMS statutory inspection stream and digital signatures</p>
          </div>
          <div class="flex items-center space-x-2">
            <a href="/statutory/form-vi" target="_blank" class="text-[11px] text-amber-400 hover:text-white bg-amber-500/10 border border-amber-500/30 hover:bg-amber-500/20 px-3 py-1.5 rounded-lg flex items-center space-x-1">
              <span>🖨️</span>
              <span>Print Ledger</span>
            </a>
            <button onclick="load()" class="text-[11px] text-slate-300 hover:text-white bg-slate-800 hover:bg-slate-700 px-3 py-1.5 rounded-lg border border-slate-700">Refresh Stream</button>
          </div>
        </div>
