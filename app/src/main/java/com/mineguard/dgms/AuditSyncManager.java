...
public class AuditSyncManager {
    ...
    public interface SyncListener {
        void onRemoteAuditReceived(String officerName, String category);
    }
            ...
            try {
                if (mqttClient != null && mqttClient.isConnected()) {
                    String payload = entry.entryId + "~~~" + entry.timestamp + "~~~" + entry.officerName 
                            + "~~~" + entry.officerId + "~~~" + entry.role + "~~~" + entry.category + "~~~" + entry.remarks;
    ...
    private void handleIncomingPayload(String raw) {
        try {
            String[] p = raw.split("~~~");
            if (p.length >= 7) {
                boolean isNew = alarmLogManager.mergeRemoteEntry(p[0], p[1], p[2], p[3], p[4], p[5], p[6]);
                if (isNew && listener != null) {
                    listener.onRemoteAuditReceived(p[2], p[5]);
                }
            }
        } catch (Throwable ignored) {}
    }
