...
public class MainActivity extends AppCompatActivity {
    ...
    private TextView tvManpowerCount, tvStrataLoad;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TextView tvOfficerName = findViewById(R.id.tv_officer_name);
    ...
    private void setupSlideToSos() {
        seekSos.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onStopTrackingTouch(SeekBar s) {
                if (s.getProgress() > 90) { // Fully Swiped
                ...
                s.setProgress(0); // Snap back
            }
        });
    }

    // Connects custom Gauge to Real Telemetry Data
    private void setupIoTTelemetryBinding() {
        ...
        safetyService.getTelemetryEngine().setListener((ch4, co, o2, airflow, isDanger) -> {
            runOnUiThread(() -> {
                gaugeCh4.updateData((float) ch4, isDanger); // Animates the Custom View
                tvSensorCh4.setText(String.format("CH4 (Methane)\n%.2f%% (%s)", ch4, isDanger ? "DANGER" : "Safe"));
                tvSensorCo.setText(String.format("CO (Carbon Monoxide)\n%d PPM", co));
                tvSensorO2.setText(String.format("O2 (Oxygen)\n%.1f%%", o2));
                
                int color = isDanger ? Color.parseColor("#EF4444") : Color.parseColor("#0EA5E9");
                tvSensorCh4.setTextColor(color);
            });
        });
    }

    // Connects Sonar Nodes UI to real BLE Mesh
    private void setupLiveMeshListeners() {
        ...
        safetyService.getBleMeshManager().setListener(new BleMeshManager.MeshListener() {
            ...
            public void onMinerUpdated(ConcurrentHashMap<String, BleMeshManager.PeerMiner> miners) {
                runOnUiThread(() -> {
                    tvPeerCount.setText(miners.size() + " Peers Linked | BLE");
                    if (miners.isEmpty()) {
                        tvSonarNodes.setText("◎ You ┄┄ 🔍 Scanning...");
                        tvNearestMiner.setText("Searching");
                        tvNearestDist.setText("");
                    } else {
                        ...
                        for (BleMeshManager.PeerMiner p : miners.values()) {
                            nodes.append(" ┄┄ ◎ ").append(p.officerName);
                            if (nearest == null || p.distanceMeters < nearest.distanceMeters) nearest = p;
                        }
                        tvSonarNodes.setText(nodes.toString());
                        
                        if (nearest != null) {
                            tvNearestMiner.setText(nearest.officerName + "\n" + nearest.officerRole);
                            tvNearestDist.setText("➔ " + nearest.distanceMeters + "m away");
                        }
                    }
                });
            }
        ...
        for (AlarmLogManager.AuditEntry e : logs) {
            if ("ATTENDANCE".equals(e.category) && e.remarks.contains("Present Miners:")) {
                try {
                    ...
                    tvManpowerCount.setText(count + " / 45 Active");
