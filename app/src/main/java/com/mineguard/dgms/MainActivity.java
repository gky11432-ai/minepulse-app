...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Action Cards (Form Interactions)
    ...
    private void setupSlideToSos() {
        ...
        seekSos.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            public void onStopTrackingTouch(SeekBar s) {
                if (s.getProgress() > 90) { // Full swipe detection
                    if (safetyService != null) {
                        // FIX: Added logic to turn OFF siren if it's already ringing
                        if (safetyService.getHardwareController().isSirenActive()) {
                            safetyService.cancelDualSos();
                            Toast.makeText(MainActivity.this, "🛑 SIREN STOPPED MANUALLY", Toast.LENGTH_SHORT).show();
                        } else {
                ...
                s.setProgress(0); // Snap back to start
            }
        });
    }
