...
public class MainActivity extends AppCompatActivity {
    ...
    private TextView tvCh4Main, tvCh4Status;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvCh4Main = findViewById(R.id.tv_ch4_main);
        tvCh4Status = findViewById(R.id.tv_ch4_status);
    ...
    private void setupIoTTelemetryBinding() {
        ...
        safetyService.getTelemetryEngine().setListener((ch4, co, o2, airflow, isDanger) -> {
            runOnUiThread(() -> {
                if (tvCh4Main != null) tvCh4Main.setText(String.format("%.2f%%", ch4));
                if (tvCh4Status != null) tvCh4Status.setText(isDanger ? "CRITICAL" : "NORMAL");
                
                if (tvSensorCh4 != null) tvSensorCh4.setText(String.format("Methane\n%.2f%%", ch4));
                if (tvSensorCo != null) tvSensorCo.setText(String.format("Carbon Monoxide\n%d PPM", co));
                if (tvSensorO2 != null) tvSensorO2.setText(String.format("Oxygen\n%.1f%%", o2));
                
                int color = isDanger ? Color.parseColor("#EF4444") : Color.parseColor("#22C55E");
                if (tvCh4Main != null) tvCh4Main.setTextColor(color);
                if (tvCh4Status != null) tvCh4Status.setTextColor(color);
                if (tvSensorCh4 != null) tvSensorCh4.setTextColor(isDanger ? Color.parseColor("#EF4444") : Color.parseColor("#0EA5E9"));
            });
        });
    }
    ...
    private void setupLiveMeshListeners() {
        ...
        safetyService.getBleMeshManager().setListener(new BleMeshManager.MeshListener() {
            ...
            public void onMinerUpdated(ConcurrentHashMap<String, BleMeshManager.PeerMiner> miners) {
                runOnUiThread(() -> {
                    if (tvPeerCount != null) tvPeerCount.setText(miners.size() + " Peers Linked");
                    if (miners.isEmpty()) {
                        if (tvSonarNodes != null) tvSonarNodes.setText("YOU --- SCANNING");
                        if (tvNearestMiner != null) tvNearestMiner.setText("Searching");
                        if (tvNearestDist != null) tvNearestDist.setText("");
                    } else {
                        StringBuilder nodes = new StringBuilder("YOU ");
                        BleMeshManager.PeerMiner nearest = null;
                        for (BleMeshManager.PeerMiner p : miners.values()) {
                            nodes.append(" --- ").append(p.officerName.toUpperCase());
        ...
        for (AlarmLogManager.AuditEntry e : logs) {
            if ("ATTENDANCE".equals(e.category) && e.remarks.contains("Present Miners:")) {
                try {
                    ...
                    if (tvManpowerCount != null) tvManpowerCount.setText(count + " Active");
