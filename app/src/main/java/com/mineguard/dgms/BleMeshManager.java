...
public class BleMeshManager {
    ...
    private byte currentRoleCode = 1;
    private String currentOfficerName = "Officer";
    ...
    public void configureOfficer(String name, String role) {
        if (name != null && !name.trim().isEmpty()) {
            this.currentOfficerName = name.trim().length() > 10 ? name.trim().substring(0, 10) : name.trim();
        }
        if (role != null) {
            if (role.contains("Sirdar")) currentRoleCode = 1;
            else if (role.contains("Overman")) currentRoleCode = 2;
            else if (role.contains("Manager")) currentRoleCode = 3;
            else if (role.contains("Safety")) currentRoleCode = 4;
            else currentRoleCode = 5;
        }
    }

    public void startOfflineMesh() { startOfflineMesh(currentOfficerName); }

    public void startOfflineMesh(String officerTag) {
        if (officerTag != null && !officerTag.isEmpty()) {
            this.currentOfficerName = officerTag.length() > 10 ? officerTag.substring(0, 10) : officerTag;
        }
        try {
            if (bluetoothAdapter == null) initBluetooth();
    ...
    public void triggerOfflineSos(String minerId) { triggerOfflineSos(); }
    ...
    public void cancelOfflineSos(String minerId) { cancelOfflineSos(); }

    private void broadcastPacket(boolean isSos) {
        try {
            if (advertiser == null && bluetoothAdapter != null) advertiser = bluetoothAdapter.getBluetoothLeAdvertiser();
            ...
            byte[] nameBytes = currentOfficerName.getBytes(StandardCharsets.UTF_8);
            byte[] payload = new byte[2 + nameBytes.length];
            payload[0] = isSos ? (byte) '!' : (byte) '#';
            payload[1] = currentRoleCode;
    ...
    private void startScanning() {
        try {
            if (scanner == null && bluetoothAdapter != null) scanner = bluetoothAdapter.getBluetoothLeScanner();
    ...
    private final ScanCallback scanCallback = new ScanCallback() {
        ...
        public void onScanResult(int callbackType, ScanResult result) {
            try {
                ...
                boolean isEmergency = (data[0] == '!');
                byte rCode = data[1];
                String role = (rCode == 1) ? "Sirdar" : (rCode == 2) ? "Overman" : (rCode == 3) ? "Manager" : "Safety Officer";
                String name = new String(data, 2, data.length - 2, StandardCharsets.UTF_8);
                ...
                // Siren rings immediately on receiving emergency packet
                if (isEmergency) {
                    hardwareController.startCollierySiren();
                }
