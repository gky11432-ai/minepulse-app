# ui_auth.py - Standalone DGMS Role-Based Access Control (RBAC) & Worker Form-B Attendance Engine

AUTH_MODULE = """
<script>
(function() {
    // Default Role Hierarchy: WORKER < OFFICER < MANAGER
    ...
    window.handleUserLogin = function(e) {
        ...
        // Officer / Manager PIN check (Default 9999 or statutory password)
        if (role !== 'WORKER') {
    ...
    // Worker Digital Self-Attendance Component (Form B - Mines Act 1952)
        ...
        card.innerHTML = `
            <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #1e293b; padding-bottom:10px; margin-bottom:12px;">
                <div style="display:flex; align-items:center; gap:8px;">
                    ...
                    <div>
                        ...
                        <div style="font-size:10px; color:#94a3b8;">Mines Act 1952 Sec 48 & CMR 48 - Pit Entry / Exit Self-Registration</div>
                    </div>
                </div>
            ...
            <form id="worker-punch-form" onsubmit="recordSelfAttendance(event)">
                <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(170px, 1fr)); gap:10px; margin-bottom:10px;">
                    ...
                    <div>
                        <label style="font-size:11px; color:#94a3b8;">Token / Biometric ID</label>
                        <input type="text" id="att-token-no" required placeholder="e.g. TK-402" style="width:100%; background:#020617; border:1px solid #334155; color:#fff; padding:8px; border-radius:6px; font-size:11px; margin-top:4px; box-sizing:border-box;">
                    </div>
                </div>
    ...
    window.recordSelfAttendance = function(e) {
        if (e) e.preventDefault();
        var type = document.getElementById('att-punch-type').value;
        var name = document.getElementById('att-miner-name').value.trim();
        var token = document.getElementById('att-token-no').value.trim();
        var shift = document.getElementById('att-shift').value;
        var district = document.getElementById('att-district').value.trim();
        var trade = document.getElementById('att-trade').value;
        var fitness = document.getElementById('att-fitness').value;

        if (fitness.indexOf('Sick') !== -1 && type === 'IN') {
            alert('⚠️ SAFETY ALERT: Worker reporting unwell/fatigued. Under CMR statutory safety rules, pit entry is withheld. Report to pithead medical room.');
            return;
        }
        ...
        var store = JSON.parse(localStorage.getItem('dgms_form_b_attendance') || '
