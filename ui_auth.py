# ui_auth.py - Standalone Role-Based Access Control & Statutory Authorization Lock
...
(function() {
    ...
    function enforcePermissions() {
        ...
        var cards = document.querySelectorAll('[id^="statutory-"]');

        cards.forEach(function(card) {
            if (card.id === 'statutory-attendance-card') return; // Workers can always use attendance

            var overlayId = card.id + '-lock-banner';
            var banner = document.getElementById(overlayId);

            if (!isOfficer) {
                if (!banner) {
                    banner = document.createElement('div');
                    banner.id = overlayId;
                    banner.style.cssText = 'background:rgba(2,6,23,0.9); border:1px dashed #eab308; border-radius:6px; padding:10px; margin-top:8px; text-align:center; color:#fef08a; font-size:11px;';
                ...
                card.querySelectorAll('input, select, textarea, button[type="submit"]').forEach(function(el) { el.disabled = true; el.style.opacity = '0.4'; });
            } else {
                if (banner) banner.remove();
                card.querySelectorAll('input, select, textarea, button[type="submit"]').forEach(function(el) { el.disabled = false; el.style.opacity = '1.0'; });
            }
        });
    }

    setInterval(enforcePermissions, 1500);
